...
- [ ] Write one canvas per context to docs/bounded-contexts/<context>.md
- [ ] Assemble and write the index docs/bounded-contexts.md (context map + discussion points)
...
3. Fill the index `assets/template.md`: purpose framing line (including the "derived from code only" note if applicable), the contexts table linking to each `bounded-contexts/<context-slug>.md` canvas, the relationships / context-map section with the mermaid diagram (built from each canvas's inbound/outbound communication and the trailing relationships blocks), and Discussion points / possible gaps. Write it to `docs/bounded-contexts.md`.
4. Create the `docs/` and `docs/bounded-contexts/` directories if needed. If any target file already exists, confirm overwrite with the user.
5. Report a short summary: number of contexts (and their canvas files), the relationships in the map, and the discussion points / unresolved TODOs to take to the team.
