...
2. Focus on the **most important** terms a domain expert would recognize and care about - core nouns, verbs, statuses, events, roles. Pitch it for an **architect bridging business and development**: keep terms verbatim (the anchor developers act on) but explainable in plain domain language (so business can validate them). Don't drift to either extreme - avoid low-level implementation detail (individual functions, methods, files, modules, internal variables, which are not ubiquitous language), and equally avoid vague abstractions that no longer point at anything concrete. A term earns a row because it names a real, shared domain concept. Do not pad the glossary with low-signal or near-duplicate technical names; surface notable variants/synonyms as discussion points instead of extra rows. When in doubt, summarize and omit rather than list. The `(evidence: <file>)` tag is just a provenance pointer - never the subject of the entry.
